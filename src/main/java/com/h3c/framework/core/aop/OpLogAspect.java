...
import com.h3c.framework.core.oplog.OpLogManager;
...
public class OpLogAspect {
	...
	@Pointcut("@annotation(com.h3c.framework.core.annotation.OpLog)")
	public void opLogPointcut() {

	}

	@Around("opLogPointcut()")
	public Object aroundOpLogPointcut(ProceedingJoinPoint joinPoint) throws H3cException {
		Object result = null;
		...
		try {
			...
			if (log != null) {
				...
				// +"---"+log.moduleName());
				if (log.value().equals(H3cLogType.RECORD)) {
					oplog.saveBizLog(log.digest(), log.moduleName());
				}
				if (log.value().equals(H3cLogType.PICTURE)) {
					if (userlog == null) {
