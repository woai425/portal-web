 ...
 * @creator     z10926<br/>
 * @create-time 2016年3月1日 上午10:13:53
 * @revision    $Id:  *
 **********************************************************************
 */
public class LicenseUtil {
	...
	public static boolean verify() throws Exception {
		...
		if ("never".equals(GlobalNames.sysConfig.get("expiration"))) {
			...
			expireDate = DateUtil.strDateToNum(GlobalNames.sysConfig
					.get("expiration"));
		}
		Long currentDate = DateUtil.strDateToNum(DateUtil
				.dateToNormalString(new java.util.Date()));
		if (currentDate <= expireDate) {
	...
	public static void parseLicenseXml() throws H3cException {
		...
		try {
			...
			for(
			Iterator<Element> i = eles.elementIterator(); i.hasNext();){
			     Element ele = i.next();
			...
			throw new H3cException("dom4j解析xml异常："+e.getMessage());
		}
	}
	...
	public static String getRandSerial() {
		...
		for (int i = 0; i < 6; i++) {
			switch (i) {
				case 0:
					sb.append(RandCodeImageEnum.UPPER_CHAR.generateStr(1)+RandCodeImageEnum.LOWER_CHAR.generateStr(1)+"-"); break;
				case 1:
					sb.append(RandCodeImageEnum.LOWER_CHAR.generateStr(1)+RandCodeImageEnum.LOWER_CHAR.generateStr(1)+"-"); break;
				case 2:
					sb.append(RandCodeImageEnum.UPPER_CHAR.generateStr(1)+RandCodeImageEnum.LOWER_CHAR.generateStr(1)+"-"); break;
				case 3:
					sb.append(RandCodeImageEnum.UPPER_CHAR.generateStr(1)+RandCodeImageEnum.UPPER_CHAR.generateStr(1)+"-"); break;
				case 4:
					sb.append(RandCodeImageEnum.LOWER_CHAR.generateStr(1)+RandCodeImageEnum.LOWER_CHAR.generateStr(1)+"-"); break;
				default:
