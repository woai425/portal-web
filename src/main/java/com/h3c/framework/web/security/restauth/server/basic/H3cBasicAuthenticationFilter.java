...
/**
 * *********************************************************************
 * 自定义基本认证鉴权过滤器，增加单点登录接口对用户名和验证的用户名进行匹配校验<br/>
 * H3cBasicAuthenticationFilter.java <br/>
 *
 * H3C所有，<br/>
 ...
 * @creator z10926 <br/>
 * @create-time 2016年11月2日 下午2:54:40
 * @revision $Id: *
 **********************************************************************
 */
public class H3cBasicAuthenticationFilter extends OncePerRequestFilter {
	 ...
	 */
	public H3cBasicAuthenticationFilter(AuthenticationManager authenticationManager) {
		...
		try {
			...
			if (request.getPathInfo().indexOf("/sso/token/") != -1) {
				if (!("/sso/token/" + username).equals(request.getPathInfo())) {
					...
					throw new BadCredentialsException("BasicAuthenticationFilter.usernameNotMatch , Username {"
							+ username
							+ "} not match");
				}
			}
