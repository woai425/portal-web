package com.h3c.framework.web.security.rsrcfilter.service.serviceimpl;
...
/**
 * *********************************************************************
 * 功能模块根据用户角色进行过滤接口实现类 RsrcFilterServiceImpl.java
 *
 * H3C所有， 受到法律的保护，任何公司或个人，未经授权不得擅自拷贝。
 * 
 * @copyright Copyright: 2015-2020
 * @creator z10926<br/>
 * @create-time 2016年2月5日 下午5:51:56
 * @revision $Id: *
 **********************************************************************
 */
@Service
public class RsrcFilterServiceImpl extends ServiceSupport implements IRsrcFilterService {
	...
	public List<Sysfunction> getNavInfoByLoginname(String... condition) throws H3cException {
		...
		if (superFlag) {
			...
			sb.append("SELECT X.FUNCTIONID,X.LOCATION,X.TITLE,X.PARENT,X.ORDERNO,X.TYPE,X.DESCRIPTION,X.PUBLICFLAG,X.ACTIVE,X.RPFLAG,X.ICONCLS,X.ICONURL from SYSFUNCTION X ");
			...
			sb.append("SELECT Y.FUNCTIONID,Y.LOCATION,Y.TITLE,Y.PARENT,Y.ORDERNO,Y.TYPE,Y.DESCRIPTION,Y.PUBLICFLAG,Y.ACTIVE,Y.RPFLAG,Y.ICONCLS,Y.ICONURL FROM ("
					...
					+ "   AND B.OBJECTID = C.GROUPID" + "   AND A.ROLEID = B.ROLEID "+" AND D.ROLEID = B.ROLEID ");
			...
			sb.append(" UNION "
					+ "SELECT X.FUNCTIONID,X.LOCATION,X.TITLE,X.PARENT,X.ORDERNO,X.TYPE,X.DESCRIPTION,X.PUBLICFLAG,X.ACTIVE,X.RPFLAG,X.ICONCLS,X.ICONURL"
					+ " FROM SYSFUNCTION X, SYSROLEACL A, SYSACT B, SYSUSER C,SYSROLE D" + " WHERE X.FUNCTIONID = A.FUNCTIONID"
					+ "   AND B.OBJECTID = C.USERID" + "   AND A.ROLEID = B.ROLEID AND D.ROLEID = B.ROLEID");
	...
	public Map<String, Integer> getFilterdBtnsByLoginname(String loginname) throws H3cException {
		...
		sb.append("SELECT X.LOCATION from SYSFUNCTION X WHERE X.ACTIVE='1' AND X.TYPE='2' AND X.BUTTONID IS NOT NULL AND X.LOCATION NOT IN (SELECT Y.LOCATION FROM ("
				+ "SELECT X.LOCATION"
